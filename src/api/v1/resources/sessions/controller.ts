...
export const getAllSessions: RequestHandler = async (req, res, next) => {
  try {
    const { sessions, totalResults } = await SessionsDal.getAllSessions(req.query);
...
export const destroySession: RequestHandler = async (req, res, next) => {
  try {
    ...
    const sessionData = await SessionsDal.getSessionByUserAndDeviceId(userId, deviceId);
    if (!sessionData) return next(new AppError("Session does not exists.", 404));
    ...
    if (sessionData.isOwner) {
      ...
      if (userSessions.length > 1) {
        return next(
          new AppError("First remove the other sessions before logging out.", 400)
        );
      }
    }

    const session = await SessionsDal.destroySession({ userId, deviceId });
    if (!session) return next(new AppError("User session does not exists.", 404));
...
export const deleteSession: RequestHandler = async (req, res, next) => {
  try {
    ...
    if (!ownerId || !ownerDeviceId || !sessionId) {
      return next(
        new AppError("Owner ID, Owner Device ID and Session ID are required.", 400)
      );
    }
    ...
    if (!ownerSessionData) return next(new AppError("User session does not exists.", 404));

    if (!ownerSessionData.isOwner) {
      return next(new AppError("You are not authorized to destroy sessions.", 403));
    }
